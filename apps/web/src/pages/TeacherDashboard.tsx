...
interface Summary {
  class_id: string; students: { id: string; display_name: string }[]; mastery: Row[]
          ...
          <tbody>
            {data.mastery.map((m) => (
              <tr key={m.student_id + m.skill_name}>
                <td><span className="avatar" aria-hidden="true">{m.display_name[0]}</span>{m.display_name} {m.has_goal_link && <span className="chip" title="You linked this learner's evidence to a plain-language IEP/504 goal label. Only you see this marker." style={{ marginLeft: 6 }}>🔗 goal link</span>}</td>
            ...
            {without.map((s) => (
              <tr key={s.id}><td><span className="avatar" aria-hidden="true">{s.display_name[0]}</span>{s.display_name}</td><td colSpan={5} className="muted">No practice evidence yet</td><td></td></tr>
            ))}
          </tbody>
        </table>
